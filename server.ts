...
async function startServer() {
  ...
  // API Route - Website Performance Scraper + PageSpeed Insights + Gemini API Report Synthesis
  app.post("/api/analyze", async (req, res): Promise<any> => {
    ...
    // Track scraped diagnostics to feed into Gemini
    let metaTitle = "";
    let metaDescription = "";
    let htmlLength = 0;
    let imagesCount = 0;
    ...
    const runPsiRequest = async (useKey: boolean, timeoutMs: number = 25000): Promise<any> => {
    ...
    try {
      ...
      if (process.env.PAGESPEED_API_KEY) {
        try {
          psiData = await runPsiRequest(true, 25000); // 25s timeout for premium key-based run
          ...
          if (isTimeout) {
            ...
            psiErrorDetails = `API query timed out after 25 seconds. ${keyedError.message}`;
          } else {
            console.warn(`PSI API with API key failed: ${keyedError.message}. Retrying keyless...`);
            psiErrorDetails = keyedError.message;
            try {
              psiData = await runPsiRequest(false, 15000); // 15s timeout for keyless fallback
              psiFetchSuccess = true;
        ...
        try {
          psiData = await runPsiRequest(false, 20000); // 20s timeout for keyless-only run
          psiFetchSuccess = true;
    ...
    // Build rich, context-aware prompt for Gemini explaining the page metrics for BOTH mobile and desktop profiles
...
`;
    ...
    try {
      ...
      console.warn("Gemini report generation failed. Instantiating ultimate fallback performance report generator:", apiError.message || apiError);
      try {
